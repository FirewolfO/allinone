...
```java
     ...
     * 事件配置key
     */
    private final String EVENT_CONFIG_KEY = "eventConfig";




    @ApiOperation("保存事件订阅接口")
    @PostMapping("/event/add")
    public String saveEventSub(@RequestBody EventSubAddReq req) {
        if (StringUtils.isBlank(req.getSubUrl())) {
            throw new MegviiException(MegviiCommonErrorCode.PARAM_INVALID);
        }
        SysConfigResp resp = sysConfigApi.queryInfoByCompanyId(companyId);
        Map<String, String> configMap = resp.getSysConfig();
        configMap.get()

        SysConfigReq req1 = new SysConfigReq();
        sysConfigApi.saveProfileInfo(req1);

        return req.getSubUrl();
    }




    @Value("${open.api.zoneId}")
