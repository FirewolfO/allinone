...
import com.firewolf.lx.rule.engine.utils.BeanUtil;
import com.firewolf.lx.rule.engine.utils.MetaInfoUtil;
import com.firewolf.lx.rule.engine.utils.sql.Limit;
import com.firewolf.lx.rule.engine.utils.sql.SqlBuildParam;
...
public class DefaultRuleService<R, I> implements IRuleService<R, I> {
    ...
    public List<R> searchRules(QueryVO data, Class<?> mainClazz, Class<?> subClazz) {
        ...
        EntityMetaInfo mainMetaInfo = MetaInfoUtil.getMetaInfo(mainClazz);
        EntityMetaInfo subMetaInfo = MetaInfoUtil.getMetaInfo(subClazz);
    ...
    private List findList(Class<?> clazz, Map<String, Object> params, int start, int size) {
        EntityMetaInfo metaInfo = MetaInfoUtil.getMetaInfo(clazz);
    ...
    public void deleteRule(Object id, Class<?> ruleClazz, Class<?> ruleItemClazz) {
        ...
        if (ruleClazz != ruleItemClazz) {
            EntityMetaInfo subMetaInfo = MetaInfoUtil.getMetaInfo(ruleItemClazz);
        ...
        //删除主表数据
        EntityMetaInfo mainMetaInfo = MetaInfoUtil.getMetaInfo(ruleClazz);
        Map<String, Object> params = new HashMap<>();
